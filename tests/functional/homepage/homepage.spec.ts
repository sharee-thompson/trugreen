...
test.describe(
  "TruGreen homepage regression",
  { tag: ["@homepage", "@functional"] },
  () => {
    test.slow();
    ...
    test("responds 200 with indexable title and canonical URL", async ({
      page,
    }) => {
    ...
    test("renders exactly one non-empty H1 hero with a primary CTA", async ({
      page,
    }) => {
    ...
    test("header exposes logo, phone and buy-online entry points", async ({
      page,
    }) => {
    ...
    test("primary navigation links point at expected destinations", async ({
      page,
    }) => {
      const header = page.locator("header").first();
    ...
    test("navigation exposes the full lawn plan and service catalog", async ({
      page,
    }) => {
      const header = page.locator("header").first();
    ...
    test("mobile header collapses to logo, call and hamburger controls", async ({
      page,
    }) => {
    ...
    test("hero Get Started opens the plan modal with Buy Online and Talk To A Pro", async ({
      page,
    }) => {
      ...
      await expect(
        modal.locator('a[href="/buy-online"]').first(),
      ).toBeVisible();
    });

    test("buy-online entry point navigates to the buy flow", async ({
      page,
    }) => {
    ...
    test("value proposition carousel renders three differentiator cards", async ({
      page,
    }) => {
    ...
    test("comparison section contrasts TruGreen, DIY and Other Guys", async ({
      page,
    }) => {
      const heading = page
    ...
    test("testimonials carousel shows rated reviews and advances", async ({
      page,
    }) => {
      const heading = page
    ...
    test("branch locator accepts a ZIP code and resolves local coverage", async ({
      page,
    }) => {
    ...
    test("contact section exposes phone, SMS and account entry points", async ({
      page,
    }) => {
      const heading = page
    ...
    test("footer renders all navigation columns with valid destinations", async ({
      page,
    }) => {
    ...
    test("footer legal, social and copyright content is present", async ({
      page,
    }) => {
      await scrollToFooter(page);
      const footer = page.locator("footer").first();
      ...
      for (const href of [
        ...
        await expect(footer.locator(`a[href="${href}"]`).first()).toHaveCount(
          1,
        );
      }
    ...
    test("all rendered images expose alt text", async ({ page }) => {
      await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
      ...
      expect(
        missingAlt,
        `images missing alt: ${missingAlt.join(", ")}`,
      ).toEqual([]);
    });

    test("page loads without console errors or failed requests", async ({
      page,
    }) => {
